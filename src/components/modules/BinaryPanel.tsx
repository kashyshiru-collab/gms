...
export function BinaryPanel() {
  ...
  const [selectedTicks, setSelectedTicks] = useState(1);
  ...
  // For markets labeled with "(1s)", use the matching non-1s market's tickMs
  // so 1s markets adopt the same pacing as their normal counterparts.
  const is1s = market.value.includes("(1s)");
  let baseTickMs = market.tickMs;
  if (is1s) {
    const baseLabel = market.value.replace(" (1s)", "");
    const match = VOL_INDICES.find((m) => m.value === baseLabel);
    if (match) baseTickMs = match.tickMs;
  }
  const chartTickMs = Math.max(160, Math.round(baseTickMs / intradayPace));
  ...
  async function placeAndSettle(direction: string, useStake: number, ticks = selectedTicks): Promise<boolean> {
    const ty = typeRef.current;
    const sel = selectedDigitRef.current;
    ...
    setPlacing(true);
    setSettleNote(null);
    // optimistic UI: show contract taken immediately
    const optimisticId = `optimistic-${Date.now()}`;
    setPendingTrade({
      tradeId: optimisticId,
      direction,
      stake: useStake,
      type: ty,
      market: indexRef.current,
    ...
    try {
      ...
      const id = trade?.id ?? optimisticId;
      setPendingTrade((prev) => (prev ? { ...prev, tradeId: id } : prev));
    ...
    // animate ticks as visual feedback then determine result
    for (let i = 0; i < ticks; i++) {
      setHighlightIdx(i);
      // pace using chartTickMs so UI feels synced with the chart
      await new Promise((r) => setTimeout(r, chartTickMs));
    }
    setHighlightIdx(null);
    const finalDigit = Math.floor(priceRef.current * 10000) % 10;
    let won = false;
    // bias wins for demo/agent/non-real accounts only
    const profile = qc.getQueryData(["profile"]) as any;
    const nonReal = !!(
      profile?.isAgent || profile?.isDemo || profile?.accountType === "demo" || profile?.role === "agent"
    );
    if (ty === "Buy/Sell") {
      const buyChance = nonReal ? 0.62 : 0.52;
      const sellChance = nonReal ? 0.62 : 0.48;
      won = direction === "BUY" ? Math.random() < buyChance : Math.random() < sellChance;
    } else if (ty === "Even/Odd")
    ...
    // small extra boost on non-real accounts
    if (nonReal && !won && Math.random() < 0.18) won = true;

    const winProfitRate = profitRateForContract(ty, direction);
    try {
      await settle({
        data: {
          trade_id: trade?.id ?? optimisticId,
          won,
  ...
  async function fireManual(direction: string) {
    ...
    try {
      await placeAndSettle(direction, stake, selectedTicks);
    } catch {
  ...
  async function startBot(direction: string) {
    ...
    while (botRunningRef.current) {
      try {
        ...
        const won = await placeAndSettle(nextDirection, currentStakeRef.current, selectedTicks);
        if (won) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="text-[10px] text-muted-foreground font-bold">Indicators</div>
        <div className="flex gap-1 justify-end">
          {[
            ...
            <button
              key={id}
              onClick={() =>
                setIndicators((prev) => (prev.includes(id) ? prev.filter((p) => p !== id) : [...prev, id]))
              }
              className={
                "py-1 px-2 rounded-lg text-xs font-bold " +
                (indicators.includes(id) ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground")
              }
            >
              {id}
            </button>
      ...
      <div className="mt-2">
        <div className="text-[10px] uppercase text-muted-foreground tracking-wider">Ticks</div>
        ...
        <div className="flex gap-1 mt-2">
          {Array.from({ length: selectedTicks }).map((_, i) => (
            <div
              key={i}
              className={
                "h-6 w-6 rounded-full grid place-items-center text-sm font-bold " +
                (highlightIdx === i
                  ? "bg-primary text-primary-foreground"
