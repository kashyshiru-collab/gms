...
export async function readSystemSettings(): Promise<SystemSettings> {
  ...
  return {
    ...
    limits_min_stake_usd: Number(data.limits_min_stake_usd ?? DEFAULT_SYSTEM_SETTINGS.limits_min_stake_usd),
    limits_max_stake_usd: Number(data.limits_max_stake_usd ?? DEFAULT_SYSTEM_SETTINGS.limits_max_stake_usd),
    volatility_model_variant: String(data.volatility_model_variant ?? DEFAULT_SYSTEM_SETTINGS.volatility_model_variant),
    user_segmentation_tags: String(data.user_segmentation_tags ?? DEFAULT_SYSTEM_SETTINGS.user_segmentation_tags),
    liability_limits_market_usd: Number(data.liability_limits_market_usd ?? DEFAULT_SYSTEM_SETTINGS.liability_limits_market_usd),
    liability_limits_user_usd: Number(data.liability_limits_user_usd ?? DEFAULT_SYSTEM_SETTINGS.liability_limits_user_usd),
    fraud_detection_enabled: Boolean(data.fraud_detection_enabled ?? DEFAULT_SYSTEM_SETTINGS.fraud_detection_enabled),
    fraud_detection_rules: String(data.fraud_detection_rules ?? DEFAULT_SYSTEM_SETTINGS.fraud_detection_rules),
    engagement_notification_triggers: String(data.engagement_notification_triggers ?? DEFAULT_SYSTEM_SETTINGS.engagement_notification_triggers),
...
export async function writeSystemSettings(
  ...
  const nextSettings: SystemSettings = {
    ...
    limits_min_stake_usd: normalizeNumber(changes.limits_min_stake_usd ?? current.limits_min_stake_usd, 1),
    limits_max_stake_usd: normalizeNumber(changes.limits_max_stake_usd ?? current.limits_max_stake_usd, 1000),
    volatility_model_variant: normalizeText(changes.volatility_model_variant ?? current.volatility_model_variant, "standard"),
    user_segmentation_tags: normalizeText(changes.user_segmentation_tags ?? current.user_segmentation_tags, "VIP,HIGH ROLLER"),
    liability_limits_market_usd: normalizeNumber(changes.liability_limits_market_usd ?? current.liability_limits_market_usd, 5000),
    liability_limits_user_usd: normalizeNumber(changes.liability_limits_user_usd ?? current.liability_limits_user_usd, 2000),
    fraud_detection_enabled: changes.fraud_detection_enabled ?? current.fraud_detection_enabled,
    fraud_detection_rules: normalizeText(changes.fraud_detection_rules ?? current.fraud_detection_rules, "bot,arbitrage"),
    engagement_notification_triggers: normalizeText(changes.engagement_notification_triggers ?? current.engagement_notification_triggers, "trade,withdrawal"),
    caps_daily_loss_usd: normalizeNumber(changes.caps_daily_loss_usd ?? current.caps_daily_loss_usd, 10000),
    caps_weekly_loss_usd: normalizeNumber(changes.caps_weekly_loss_usd ?? current.caps_weekly_loss_usd, 50000),
...
function getSegmentationMultiplier(
  ...
  if (tags.includes("HIGH ROLLER") && (depositsUsd >= 5000 || volumeUsd >= 15000 || tradeCount >= 100)) {
    return 1.5;
  }
