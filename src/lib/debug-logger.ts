...
const STORAGE_KEY = "tronix-option.debug.events";
const ENABLED_KEY = "tronix-option.debug.enabled";
...
export function logDebugEvent(level: DebugLevel, scope: string, message: string, data?: unknown) {
  ...
  window.dispatchEvent(new CustomEvent("tronix-option-debug-log", { detail: event }));
  return event;
}
...
export function clearDebugEvents() {
  ...
  window.dispatchEvent(new CustomEvent("tronix-option-debug-log-cleared"));
}
