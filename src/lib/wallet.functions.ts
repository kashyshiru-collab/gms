...
export const initiateDeposit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { stkPush, publicAppUrl } = await import("./payhero.server");
    const callbackUrl = `${publicAppUrl()}/api/public/payhero/callback`;
    ...
    try {
      ...
      const payheroRef = resp?.reference ?? resp?.CheckoutRequestID ?? null;
      if (payheroRef) {
        await supabase
          .from("transactions")
          .update({ payhero_reference: payheroRef })
...
export const reconcilePendingDeposits = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: pendings } = await supabase
      .from("transactions")
      .select("id, reference, payhero_reference, amount_kes, meta, created_at")
      .eq("user_id", userId)
      .eq("type", "deposit")
      .eq("status", "pending")
    ...
    const { getTransactionStatus } = await import("./payhero.server");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

    let credited = 0, failed = 0;
    for (const tx of pendings) {
      const lookup = tx.payhero_reference || tx.reference;
      if (!lookup) continue;
      try {
        const r: any = await getTransactionStatus(lookup);
        ...
        if (status === "SUCCESS") {
          ...
          await supabaseAdmin.from("transactions").update({
            status: "success",
            mpesa_receipt: r?.provider_reference ?? r?.third_party_reference ?? null,
