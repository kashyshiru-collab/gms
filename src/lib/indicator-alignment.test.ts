...
export function testIndicatorAlignment() {
  const prices = [100, 101, 102, 101, 100, 101, 102, 103, 102, 101, 102, 103, 104, 103, 102, 101, 102, 103, 104, 105];
  ...
  console.log('✓ All values filled:', smaAligned.every(v => v !== null && v !== undefined));
  ...
  console.log('✓ All values filled:', emaAligned.every(v => v !== null && v !== undefined));
  ...
  console.log('✓ All values filled:', rsiAligned.every(v => v !== null && v !== undefined));
  console.log('✓ Same length as prices:', rsiAligned.length === prices.length);
  console.log('✓ RSI in valid range [0,100]:', rsiAligned.every(v => v >= 0 && v <= 100));
  ...
  console.log('✓ Upper all filled:', bbAligned.upper.every(v => v !== null && v !== undefined));
  console.log('✓ Lower all filled:', bbAligned.lower.every(v => v !== null && v !== undefined));
  console.log('✓ Upper > Lower:', bbAligned.upper.every((u, i) => u > bbAligned.lower[i]));
  ...
  console.log('✓ MACD all filled:', macdAligned.macd.every(v => v !== null && v !== undefined));
  console.log('✓ Signal all filled:', macdAligned.signal.every(v => v !== null && v !== undefined));
}
...
export function testChartPathAlignment() {
  ...
  console.log('✓ All Y values valid:', pathPoints.every(p => p.y >= 0 && p.y <= height));
}
...
export function testWarmupPeriodHandling() {
  ...
  console.log('✓ Forward fill works:', smaAligned2.slice(0, 5).every(v => v === smaAligned2[19]));
}
