...
function Dashboard() {
  ...
  const openMut = useMutation({
    ...
    onSuccess: (_, vars) => {
      toast.success(`${vars.side.toUpperCase()} ${symbol} for ${fmtUSD(Number(stake))} opened`);
      qc.invalidateQueries({ queryKey: ["dash"] });
    },
  ...
  const closeMut = useMutation({
    ...
    onSuccess: (r) => {
      toast[r.pnl >= 0 ? "success" : "warning"](
        `Closed @ ${r.exit.toFixed(5)} · P&L ${fmtUSD(r.pnl)}`,
      );
      qc.invalidateQueries({ queryKey: ["dash"] });
    },
    onError: (e: Error) => toast.error(e.message),
              ...
              <tbody className="divide-y divide-border tabular">
                ...
                {(dashQ.data?.positions ?? []).map((p: any) => (
                  <tr key={p.id}>
                    ...
                    <td className="px-4 py-2">{fmtUSD(Number(p.stake_kes))}</td>
                    ...
                    >
                      {p.pnl_kes == null ? "—" : fmtUSD(Number(p.pnl_kes))}
                    </td>
      ...
      <DialogContent>
        ...
        {step === "form" && (
          <DialogFooter className="flex-row justify-between sm:justify-between">
            ...
            <Button onClick={submit} disabled={loading || !phone || Number(amount) < MIN_DEPOSIT_USD}>
              {loading ? "Sending…" : `Send STK for ${fmtUSD(Number(amount) || 0)}`}
            </Button>
          </DialogFooter>
        )}
      </DialogContent>
