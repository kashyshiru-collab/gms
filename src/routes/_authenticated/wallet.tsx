...
import { Plus, Minus, History, Smartphone, Bitcoin, LogOut, ArrowDownLeft, ArrowUpRight, Clock } from "lucide-react";
...
function WalletPage() {
  ...
  const balUSD = activeAccount === "real" ? Number(profile?.balance_usd ?? 0) : Number(profile?.demo_balance_usd ?? 0);
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl bg-gradient-to-br from-primary/20 to-primary-glow/10 border border-primary/30 p-5 text-center glow-primary">
        ...
        <div className="text-xs text-muted-foreground mt-0.5">~ KSh {(balUSD * 130).toFixed(0)}</div>
      </div>

      <div className="grid grid-cols-3 gap-1 bg-card border border-border rounded-xl p-1">
        {([["deposit", Plus], ["withdraw", Minus], ["history", History]] as const).map(([k, Icon]) => (
          <button key={k} onClick={() => setTab(k)}
            className={"py-2.5 rounded-lg flex flex-col items-center gap-0.5 text-xs font-semibold " + (tab === k ? "bg-primary/15 text-primary" : "text-muted-foreground")}>
      ...
      {(tab === "deposit" || tab === "withdraw") && (
        <>
          <div className="grid grid-cols-2 gap-2">
            <MethodCard active={method === "mpesa"} onClick={() => setMethod("mpesa")} icon={<Smartphone className="h-5 w-5" />} title="M-Pesa" sub="STK Push" />
            <MethodCard active={method === "crypto"} onClick={() => setMethod("crypto")} icon={<Bitcoin className="h-5 w-5" />} title="Crypto" sub="USDT / BTC / ETH" />
          </div>

          <div>
            <div className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground mb-1">Amount ({method === "mpesa" ? "KSH" : "USD"})</div>
            <div className="flex items-center bg-card border border-border rounded-xl px-3 py-2.5">
              <span className="font-bold text-muted-foreground mr-2 text-sm">{method === "mpesa" ? "KSh" : "$"}</span>
          ...
          {method === "mpesa" && (
            <div>
              <div className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground mb-1">Safaricom Number</div>
              <div className="flex items-center bg-card border border-border rounded-xl px-3 py-2.5">
                <span className="mr-2 text-xs font-bold text-muted-foreground">KE</span>
                <input value={phone} onChange={(e) => setPhone(e.target.value)} className="flex-1 bg-transparent outline-none font-bold tabular-nums text-sm" />
              </div>
            </div>
          )}

          <button onClick={tab === "deposit" ? deposit : withdraw} disabled={busy}
            className={"w-full py-3 rounded-xl font-bold text-sm disabled:opacity-50 " + (tab === "deposit" ? "bg-bull text-bull-foreground glow-bull" : "bg-primary text-primary-foreground glow-primary")}>
            {busy ? "Processing..." : tab === "deposit" ? `Deposit ${method === "mpesa" ? "KSh" : "$"}${amount || 0}` : `Withdraw ${method === "mpesa" ? "KSh" : "$"}${amount || 0}`}
          </button>
        </>
      )}
          ...
          {history.map((t) => {
            ...
            return (
              <div key={t.id} className="flex items-center gap-3 p-3">
                <div className={"h-9 w-9 rounded-lg grid place-items-center " + (isDeposit ? "bg-bull/15 text-bull" : "bg-bear/15 text-bear")}>
                  {isDeposit ? <ArrowDownLeft className="h-4 w-4" /> : <ArrowUpRight className="h-4 w-4" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm capitalize">{t.kind} - {t.method ?? "system"}</div>
                ...
                <div className="text-right">
                  <div className={"font-bold tabular-nums text-sm " + (isDeposit ? "text-bull" : "text-bear")}>
                    {isDeposit ? "+" : "-"}{t.currency === "KSH" ? "KSh" : "$"}{Number(t.amount).toFixed(2)}
                  </div>
      ...
      <button onClick={signOut} className="w-full py-2.5 rounded-xl bg-card border border-border text-bear font-semibold flex items-center justify-center gap-2 mt-4 text-sm">
...
function MethodCard({ active, onClick, icon, title, sub }: { active: boolean; onClick: () => void; icon: React.ReactNode; title: string; sub: string }) {
  return (
    <button onClick={onClick}
      className={"py-2.5 rounded-xl border flex items-center gap-2 px-3 " + (active ? "bg-primary/15 border-primary glow-primary" : "bg-card border-border")}>
