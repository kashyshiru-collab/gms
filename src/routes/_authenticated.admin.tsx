...
function readPayheroBalance(wallet: unknown) {
  if (!wallet || typeof wallet !== "object") return 0;
  const row = wallet as Record<string, unknown>;
  return Number(row.service_wallet_balance ?? row.balance ?? row.amount ?? 0);
}
...
function AdminPage() {
          ...
          <div className="grid gap-4 md:grid-cols-[320px_1fr] md:items-end">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <Metric
                ...
                value={fmtKES(reportQ.data?.selected?.totalDeposited ?? 0)}
              />
            </div>
          </div>
        </Card>
                ...
                {(reportQ.data?.agents ?? []).map((agent) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-right tabular">
                      {fmtKES(Number(agent.totalDeposited ?? 0))}
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {(reportQ.data?.clients ?? []).map((client) => (
                  <tr key={client.id} className="border-b border-border/60">
                    ...
                    <td className="p-3 text-right tabular">{fmtKES(client.deposited)}</td>
          ...
          <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              <Metric
                icon={ArrowDownToLine}
                label="Deposited"
                value={fmtKES(summary?.deposited ?? 0)}
              />
              <Metric icon={Banknote} label="Withdrawn" value={fmtKES(summary?.withdrawn ?? 0)} />
              <Metric
                icon={Landmark}
                label="Remaining"
                value={fmtKES(summary?.remainingBalance ?? 0)}
              />
              <Metric icon={Shield} label="Locked" value={fmtKES(summary?.nonWithdrawable ?? 0)} />
              <Metric
                icon={Wallet}
                label="Withdrawable"
                value={fmtKES(summary?.withdrawableBalance ?? 0)}
              />
            </div>
          </div>
        </Card>
              ...
              <tbody>
                {((financeQ.data?.transactions ?? []) as Tx[]).map((tx) => (
                  <tr key={tx.id} className="border-b border-border/60">
                    ...
                    <td className="p-3 text-right tabular">{fmtKES(Number(tx.amount_kes))}</td>
  ...
  function OverviewPanel({ overview }: { overview: AdminOverview | undefined }) {
    ...
    const payheroBalance = readPayheroBalance(overview?.payheroWallet);
    const withdrawMut = useMutation({
      mutationFn: (vars: { amount: number; phone: string }) => withdrawFn({ data: vars }),
      onSuccess: () => {
        toast.success("Payhero withdrawal initiated");
        setAmount("");
        qc.invalidateQueries({ queryKey: ["admin-overview"] });
      },
      onError: (error: Error) => toast.error(error.message),
    ...
    return (
      <div className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Metric icon={Wallet} label="Payhero merchant wallet" value={fmtKES(payheroBalance)} />
          <Metric
            icon={Landmark}
            label="Total user balances"
            value={fmtKES(overview?.totalUserBalance ?? 0)}
          />
          <Metric
            icon={Shield}
            label="Locked admin credits"
            value={fmtKES(overview?.totalLockedBalance ?? 0)}
          />
          <Metric
            icon={Users}
            label="Users / agents"
        ...
        <Card className="space-y-4 p-5">
          <div>
            <h2 className="font-semibold">Withdraw Payhero wallet to M-Pesa</h2>
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
            ...
            <div>
              <Label htmlFor="admin-amount">Amount (KES)</Label>
              <Input
                id="admin-amount"
                type="number"
                value={amount}
...
function PersonCells({ person }: { person: Person }) {
  return (
    <>
      ...
      <td className="p-3 text-right tabular">{fmtKES(person.balance)}</td>
      <td className="p-3 text-right tabular">{fmtKES(person.nonWithdrawable)}</td>
      <td className="p-3 text-right tabular">{fmtKES(person.withdrawable)}</td>
    </>
  );
}
...
function SecretAdminLogo({ onExit }: { onExit: () => void }) {
  ...
  return (
    <button onClick={onTap} className="flex items-center gap-2" aria-label="GMX Trader admin">
      <img src="/gmx-logo.png" alt="" width={30} height={30} className="h-8 w-8 rounded-md" />
