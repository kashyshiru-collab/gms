  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (ref) {
          try { await attachFn({ data: { code: ref } }); } catch (e) { console.warn("attach referrer failed", e); }
        }
      ...
      <div className="w-full max-w-sm rounded-xl border border-border bg-card p-8">
        ...
        {ref && mode === "signup" && (
          <div className="mt-3 rounded-md border border-bull/40 bg-bull/10 px-3 py-2 text-xs">
            Invited by code <span className="font-mono font-semibold">{ref.toUpperCase()}</span> · you'll be linked to their team.
          </div>
        )}
