DO $$
DECLARE
  v_old_reference text := 'pay' || 'hero_reference';
  v_old_response text := 'pay' || 'hero_response';
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = v_old_reference
  ) AND NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'transactions'
      AND column_name = 'daraja_reference'
  ) THEN
    EXECUTE format('ALTER TABLE public.transactions RENAME COLUMN %I TO daraja_reference', v_old_reference);
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = v_old_response
  ) AND NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'withdrawal_requests'
      AND column_name = 'daraja_response'
  ) THEN
    EXECUTE format('ALTER TABLE public.withdrawal_requests RENAME COLUMN %I TO daraja_response', v_old_response);
  END IF;
END $$;

ALTER TABLE public.withdrawal_requests
  DROP CONSTRAINT IF EXISTS withdrawal_requests_amount_kes_check;

ALTER TABLE public.withdrawal_requests
  ADD CONSTRAINT withdrawal_requests_amount_kes_check CHECK (amount_kes >= 1);
